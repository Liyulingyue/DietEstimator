...
def get_prompt_bowel_single_image_analysis():
    """生成单张粪便图片分析的prompt"""
    example_dict = {
        "颜色": "str，粪便的具体颜色，如黄褐色、深褐色、黑色、红色等",
        "份量": "str，粪便的份量，选项：少/适中/多",
        "形态": "str，粪便的形态，选项：颗粒型/糊状/条状/块状/水样",
        "健康点评": "str，对肠道健康的综合点评和建议",
        "分析依据": "str，详细的分析理由和依据"
    }
...
2. 使用```json```包裹输出内容。
3. 不要在输出的json中增加注释。
4. 严格按照给定的选项填写颜色、份量、形态字段。
5. 健康点评要专业、客观，提供合理建议。
6. 分析依据要详细说明观察到的特征和判断依据。
